...
def get_DF_optionchain(ticker):
  ...
  for e in exps:
    try:
        ...
        combined[['bid', 'ask', 'strike']] = combined[['bid', 'ask', 'strike']].apply(pd.to_numeric)
        combined['mark'] = (combined['bid'] + combined['ask']) / 2

        combined['pricepercent'] = 100 * combined[bias_price] / lastprice
        combined['pricepercentstrike'] = 100 * combined[bias_price] / combined['strike']
        combined['interinsicvalue'] = combined.apply(lambda row: abs(lastprice - row['strike']) if row['inTheMoney'] else 0, axis=1)
        combined['interinsicvalue%'] = 100*combined['interinsicvalue'] / combined[bias_price]
        combined['timevalue'] = combined.apply(lambda row: row[bias_price] - row['interinsicvalue'], axis=1)
        combined['timevalue%'] = 100*combined['timevalue'] / combined[bias_price]
        combined['breakevenprice'] = combined.apply(lambda row: row[bias_price]+ row['strike'] if row['Type']=='CALL' else row['strike'] - row[bias_price], axis=1)
