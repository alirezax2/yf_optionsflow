...
def get_chain_greeks_date(
    stock_ticker, dividend_yield, option_type, expiration_date, risk_free_rate=None
):
...
def __greeks(data, chain, option_type, r=None, dividend_yield=None):
    ...
    for i in chain:
        ...
        try:
            last_traded.append(
                str(
                    datetime.datetime.utcfromtimestamp(i["lastTradeDate"]).strftime(
                        "%Y-%m-%d %I:%M:%S %p"
                    )
                )
            )
        ...
        t = (
            expiration_date + datetime.timedelta(seconds=17.5 * 3600) - today
        ).days / 365
        ...
        if dividend_yield is not None:
            if option_type == "c":
                if v != 0:
                    d1 = (
                        log(float(underlying_price) / strike[-1])
                        + ((r - dividend_yield) + v * v / 2.0) * t
                    ) / (v * t_sqrt)
                    d2 = d1 - v * t_sqrt
                    delta.append(round(norm.cdf(d1), 4))
                    gamma.append(
                        round(norm.pdf(d1) / (underlying_price * v * t_sqrt), 4)
                    )
                    theta.append(
                        round(
                            (
                                -(underlying_price * v * norm.pdf(d1)) / (2 * t_sqrt)
                                - r * strike[-1] * exp(-r * t) * norm.cdf(d2)
                            )
                            / 365,
                            4,
                        )
                    ...
                    vega.append(
                        round(underlying_price * t_sqrt * norm.pdf(d1) / 100, 4)
                    )
                    rho.append(
                        round(strike[-1] * t * exp(-r * t) * norm.cdf(d2) / 100, 4)
                    )
                else:
                    delta.append(0)
                    gamma.append(0)
            ...
            if option_type == "p":
                d1 = (log(float(underlying_price) / strike[-1]) + r * t) / (
                    v * t_sqrt
                ) + 0.5 * v * t_sqrt
                d2 = d1 - (v * t_sqrt)
                delta.append(round(-norm.cdf(-d1), 4))
...
def __to_timestamp(date):
    return str(
        calendar.timegm((datetime.datetime.strptime(date, "%Y-%m-%d")).utctimetuple())
    )
